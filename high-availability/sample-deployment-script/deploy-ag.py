...
def create_operator_yaml(namespace,
    ...
    with open(operator_template) as f:
        operator_yaml = OperatorYaml(yaml.load_all(f.read(), Loader=yaml.FullLoader))
...
def create_pv_yaml(namespace,
    ...
    with open(pv_template) as f:
        pv_yaml_template = PersistentVolumeYaml(yaml.load(f.read(), Loader=yaml.FullLoader))
        pv_yaml_list = []
...
#     with open(pv_template) as f:
#         pvc_yaml_template = PersistentVolumeClaimYaml(yaml.load(f.read(), Loader=yaml.FullLoader))
...
def create_sqlservers_yaml(env,
    ...
    with open(sqlserver_template) as f:
        sqlserver_yaml_template = SqlServerYaml(yaml.load_all(f.read(), Loader=yaml.FullLoader))
        sqlserver_yaml_list = []
        for sqlserver_name in sqlservers:
...
def create_ag_services_yaml(env,
    ...
    with open(ag_service_template) as f:
        ag_service_yaml = AgServiceYaml(yaml.load(f.read(), Loader=yaml.FullLoader))
        if env is Environment.ON_PREM:
...
def create_sql_secrets_yaml(namespace,
    ...
    with open(sql_secrets_template) as f:
        sql_secrets_yaml = SqlSecretsYaml(yaml.load(f.read(), Loader=yaml.FullLoader))
...
def create_failover_yaml(namespace,
    ...
    with open(failover_template) as f:
        failover_yaml = FailoverYaml(yaml.load_all(f.read(), Loader=yaml.FullLoader))
